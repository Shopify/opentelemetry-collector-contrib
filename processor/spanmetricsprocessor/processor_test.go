...
func TestProcessorConcurrentShutdown(t *testing.T) {
	...
	// Test
	p := newProcessorImp(mexp, tcon, nil, cumulative, "foo", logger, ticker)
...
func TestProcessorConsumeTracesErrors(t *testing.T) {
	...
	p := newProcessorImp(mexp, tcon, nil, cumulative, "foo", logger, nil)

	traces := buildSampleTrace(0)
...
func TestProcessorConsumeTraces(t *testing.T) {
	...
	testcases := []struct {
		...
		},
		{
			...
			traces:                 []ptrace.Traces{buildSampleTrace(0), buildSampleTrace(0)},
		},
		{
		...
		{
			// Consumptions with spans generated by latency sli service
			name:                   "Test latency SLI",
			aggregationTemporality: cumulative,
			verifier:               verifyLatencyGaugeOkay,
			traces:                 []ptrace.Traces{buildIngestLatencySampleTrace(0)},
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			p := newProcessorImp(mexp, tcon, &defaultNullValue, tc.aggregationTemporality, tc.ingestLatencyServiceName, zaptest.NewLogger(t), ticker)
...
func TestMetricKeyCache(t *testing.T) {
	...
	defaultNullValue := pcommon.NewValueStr("defaultNullValue")
	p := newProcessorImp(mexp, tcon, &defaultNullValue, cumulative, "foo", zaptest.NewLogger(t), nil)
	traces := buildSampleTrace(0)
...
func BenchmarkProcessorConsumeTraces(b *testing.B) {
	...
	defaultNullValue := pcommon.NewValueStr("defaultNullValue")
	p := newProcessorImp(mexp, tcon, &defaultNullValue, cumulative, "foo", zaptest.NewLogger(b), nil)
...
func newProcessorImp(mexp *mocks.MetricsExporter, tcon *mocks.TracesConsumer, defaultNullValue *pcommon.Value, temporality string, ingestLatencyServiceName string, logger *zap.Logger, ticker *clock.Ticker) *processorImp {
	...
	return &processorImp{
		logger:          logger,
		config:          Config{AggregationTemporality: temporality, IngestLatencyServiceName: ingestLatencyServiceName},
...
func verifyConsumeMetricsInputCumulative(t testing.TB, input pmetric.Metrics) bool {
	return verifyConsumeMetricsInput(t, input, pmetric.AggregationTemporalityCumulative, 1, 1)
}

func verifyConsumeMetricsInputPVal(t testing.TB, input pmetric.Metrics) bool {
	return verifyConsumeMetricsInput(t, input, pmetric.AggregationTemporalityCumulative, 1, 2)
}

func verifyLatencyGaugeOkay(t testing.TB, input pmetric.Metrics) bool {
	rm := input.ResourceMetrics()
	assert.NotZero(t,
		rm.At(0).ScopeMetrics().At(0).Metrics().At(0).Gauge().DataPoints().At(0).IntValue(),
	)
	return true
}
...
func verifyConsumeMetricsInputDelta(t testing.TB, input pmetric.Metrics) bool {
	return verifyConsumeMetricsInput(t, input, pmetric.AggregationTemporalityDelta, 1, 1)
}
...
func verifyMultipleCumulativeConsumptions() func(t testing.TB, input pmetric.Metrics) bool {
	...
	return func(t testing.TB, input pmetric.Metrics) bool {
		...
		return verifyConsumeMetricsInput(t, input, pmetric.AggregationTemporalityCumulative, numCumulativeConsumptions, 1)
	}
}
...
func verifyConsumeMetricsInput(t testing.TB, input pmetric.Metrics, expectedTemporality pmetric.AggregationTemporality, numCumulativeConsumptions, adjustedSampleCount int) bool {
	...
	for dpi := 0; dpi < 3; dpi++ {
		dp := callsDps.At(dpi)
		assert.Equal(t, int64(numCumulativeConsumptions*adjustedSampleCount), dp.IntValue(), "There should only be one metric per Service/operation/kind combination")
	...
	for dpi := 0; dpi < 3; dpi++ {
		...
		for bi := 0; bi < dp.BucketCounts().Len(); bi++ {
			...
			if bi == foundLatencyIndex {
				wantBucketCount = uint64(numCumulativeConsumptions * adjustedSampleCount)
			}
...
func buildSampleTrace(pval int) ptrace.Traces {
	...
	initServiceSpans(
		...
		}, traces.ResourceSpans().AppendEmpty(), pval)
	initServiceSpans(
		serviceSpans{
	...
	initServiceSpans(serviceSpans{}, traces.ResourceSpans().AppendEmpty(), pval)
	return traces
}

func buildIngestLatencySampleTrace(pval int) ptrace.Traces {
	traces := ptrace.NewTraces()

	initServiceSpans(
		serviceSpans{
			serviceName: "my-cool-latency-metric-service",
			spans: []span{
				{
					operation:  "/ping",
					kind:       ptrace.SpanKindServer,
...
func initServiceSpans(serviceSpans serviceSpans, spans ptrace.ResourceSpans, pval int) {
	...
	for _, span := range serviceSpans.spans {
		initSpan(span, ils.Spans().AppendEmpty(), pval)
	}
}

func initSpan(span span, s ptrace.Span, pval int) {
	...
	if pval != 0 {
		s.TraceState().FromRaw(fmt.Sprintf("ot=p:%d;r:9", pval))
	}
}
...
func TestProcessorUpdateExemplars(t *testing.T) {
	...
	// ----- call -------------------------------------------------------------
	p.updateHistogram(key, value, traceID, spanID, uint64(1))
...
func TestConsumeTracesEvictedCacheKey(t *testing.T) {
	...
	initServiceSpans(
		...
		}, traces0.ResourceSpans().AppendEmpty(), 0)
	initServiceSpans(
		serviceSpans{
			serviceName: "service-b",
			spans: []span{
	...
	initServiceSpans(
		...
		}, traces1.ResourceSpans().AppendEmpty(), 0)
	initServiceSpans(
		serviceSpans{
			serviceName: "service-c",
			spans: []span{
	...
	p := newProcessorImp(mexp, tcon, &defaultNullValue, cumulative, "foo", zaptest.NewLogger(t), ticker)
